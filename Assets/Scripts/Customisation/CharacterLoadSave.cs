...
namespace Customisation
{
    public class CharacterLoadSave : MonoBehaviour
    {
        ...
        private void Start()
        {
            uiChannel.OnLoadCharacterData?.Invoke(characterSaveData, currentItemType,
                customisationItems.AllGenderItemTypeDataList);
        }
        ...
        private void Load()
        {
            ...
            foreach (ItemTypeData itemTypeData in currentItemType)
            {
                ...
                ItemDataFromList(currentItemType, itemTypeData.itemType)[CurrentIndex(itemTypeData.itemType)].item
                    .SetActive(true);
            }

            foreach (ItemTypeData itemTypeData in customisationItems.AllGenderItemTypeDataList)
            {
                ItemDataFromList(customisationItems.AllGenderItemTypeDataList, itemTypeData.itemType)[
                    CurrentIndex(itemTypeData.itemType)].item.SetActive(true);
            }

            if (characterSaveData.elements == Elements.No)
            {
                ItemDataFromList(currentItemType, ItemType.All_Elements)[CurrentIndex(ItemType.All_Elements)].item
                    .SetActive(false);
                ItemDataFromList(currentItemType, ItemType.No_Elements)[CurrentIndex(ItemType.No_Elements)].item
                    .SetActive(true);
            }
            else
            {
        ...
        {
            foreach (IndexData selectedItem in characterSaveData.selectedItems)
            {
                ...
                characterSaveData.openedItems.Add(currentOpenedItemsType);
            }
        }

        private int CurrentIndex(ItemType itemType)
        {
            return characterSaveData.selectedItems.Find(e => e.itemType == itemType).currentIndex;
        }
